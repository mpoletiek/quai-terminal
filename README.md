...
- **Private payments (BIP47 payment codes):** send to a code, peers and channel scans. The recipient's wallet finds a payment only once it knows your code: if they have not been told it, the Send Qi form's *Announce* (on by default; `send qi --notify` on the command line) follows the send with a mailbox announcement, a small public QUAI transaction from the account in use. Otherwise they add your code themselves (`payment add`). While unlocked, the TUI and daemon read the Pelagus mailbox every ~90 s and rescan known channels for later payments (`payment sync` does it on demand). Mailbox announcements are unauthenticated, so a sender you have never added is not registered on its own: if Qi is waiting on its channel it becomes an **offer** in People › Contacts, in the channels pane (`payment offers`), which you accept (`enter`, `payment accept CODE`) or decline (`x`, `payment decline CODE`). You are notified of an offer once 1 Qi or more is waiting; smaller offers are listed quietly.
