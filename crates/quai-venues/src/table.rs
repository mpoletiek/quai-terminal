...
//!
//! Adding a UniswapV2 exchange is a [`Venue`] variant, a row in [`AMMS`], and its pins in
//! [`Ecosystem`]: this crate only.

use crate::Venue;
use crate::capabilities::{Action, Family, Support};
use crate::pins::{Ecosystem, PinnedContract};
...
pub trait VenueKind: Sync {
    ...
    fn pins<'a>(&self, network: &'a Ecosystem) -> Option<(&'a PinnedContract, &'a PinnedContract)>;
...
pub struct Amm {
    ...
    router: fn(&Ecosystem) -> Option<&PinnedContract>,
    factory: fn(&Ecosystem) -> Option<&PinnedContract>,
}

impl Amm {
    /// Its factory on a network.
    pub fn factory<'a>(&self, network: &'a Ecosystem) -> Option<&'a PinnedContract> {
        (self.factory)(network)
    }

    /// Its router on a network.
    pub fn router<'a>(&self, network: &'a Ecosystem) -> Option<&'a PinnedContract> {
        (self.router)(network)
    }
}
...
pub static AMMS: [Amm; 4] = [
    Amm {
        ...
        router: |e| e.quainance_router.as_ref(),
        factory: |e| e.quainance_factory.as_ref(),
    },
    Amm {
        ...
        router: |e| e.launch_amm_router.as_ref(),
        factory: |e| e.launch_amm_factory.as_ref(),
    },
    Amm {
        venue: Venue::Legacy,
        label: "QuaiSwap",
        ...
        router: |e| e.legacy_router.as_ref(),
        factory: |e| e.legacy_factory.as_ref(),
    },
    Amm {
        ...
        router: |e| e.hartii_amm_router.as_ref(),
        factory: |e| e.hartii_amm_factory.as_ref(),
    },
];
...
impl VenueKind for Curve {
    ...
    fn pins<'a>(&self, _network: &'a Ecosystem) -> Option<(&'a PinnedContract, &'a PinnedContract)> {
        None
    }
