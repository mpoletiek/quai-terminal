...
/// Seal a v2 message posted at `block`. Nothing writes v2 any more ([`crate::wire`] replaced
...
pub fn sealed_lines(c: &Conversation, posts: &[Post], mine: &[String], known: &[String]) -> Vec<SealedLine> {
    ...
    for p in posts {
        ...
        lines.push(SealedLine { at: p.at, from: p.from.clone(), mine, text, new_address });
    }
    lines
}
