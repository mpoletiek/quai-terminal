...
pub enum Ev {
    ...
    Messaging {
        epoch: u64,
        view: Result<crate::messaging::MessagingView, String>,
...
impl Worker {
    ...
    pub fn send(&self, cmd: Cmd) {
        match cmd {
            ...
            // The keys go to the worker alone: every lane's session of this wallet shares its
            // custody (`wallet_core::custody`), so there is nothing to copy to the others.
            Cmd::UseKeys { wallet, keys } => {
                let _ = self.tx.send(Cmd::UseKeys { wallet, keys });
            }
            Cmd::Lock => {
...
pub fn lane_thread(name: &str) -> std::thread::Builder {
            ...
            while let Ok(job) = rx.recv() {
                ...
                match job {
                    ...
                    SignJob::Network(id) => {
                        ...
                        // keys already held. Only this lane's reviews go.
                        if let Some(s) = session.as_mut() {
                            s.close();
                        }
                        network = id;
...
impl MsgLane {
    fn spawn(registry: Registry, config: AppConfig, events: Sender<Ev>) -> MsgLane {
        ...
        let _ = lane_thread("wallet-messages").spawn(move || {
            ...
            while let Ok(job) = rx.recv() {
                ...
                // This lane's reviews go between jobs; the keys stay the wallet's.
                session.close();
    ...
    fn ask(&self, session: &Session, work: MsgWork, epoch: u64) {
        let key = QiKey { wallet: session.meta.id.clone(), network: session.network.id.clone() };
        let _ = self.jobs.send(MsgJob { key, meta: session.meta.clone(), work, epoch });
    }
}
    ...
    loop {
        ...
        match cmd {
            Cmd::Order(request) => {
                ...
                let quiet = matches!(request, crate::orders::Request::Watch);
                if !quiet {
                    send(Ev::Busy(Some("checking limit orders…".into())));
                }
                match crate::orders::handle(&mut session, request).await {
                    Ok(event) => send(event),
            ...
            Cmd::SwitchNetwork(net) => {
                ...
                match open(&meta, &net) {
                    Ok(s) => {
                        // Same wallet: the new network's session shares its custody, so the keys
                        // are still there. Only the old session's reviews go.
                        session.close();
                        session = s;
                        if let Some(note) = session.use_monitor().await {
                            send(Ev::Info(note));
            ...
            Cmd::ChatNews { dms_only, epoch } => match session.is_unlocked() {
                true => messages.ask(&session, MsgWork::News { dms_only }, epoch),
                false => {
...
pub fn incoming_amount(a: &Activity) -> String {
...
async fn messaging_view(session: &Session, sync: bool) -> Result<crate::messaging::MessagingView, String> {
    ...
    if !matches!(status.need, wallet_core::messaging::service::KeyNeed::NotSetUp | wallet_core::messaging::service::KeyNeed::NoKeys) {
        ...
        return Ok(crate::messaging::MessagingView { status, conversations, requests });
    }
    Ok(crate::messaging::MessagingView { status, conversations: Vec::new(), requests: Vec::new() })
}
